...
CACHE_DIR = 'year_files/cache_2024'

CATASTROPHIC_DATES = {20240309}
...
def run_repair_2024(file_path=TARGET_FILE, year=2024, ps=False):
    ...
    for idx, date_num in enumerate(remaining_dates, 1):
        ...
        print(f"[{idx}/{len(remaining_dates)}] Fetching Date {date_num} (Catastrophic={is_catastrophic})...")

        # --- A. BASELINE MISSING ENDPOINTS (ALL 160 DATES) ---
        # 1. Pullups
        ...
        # --- B. CATASTROPHIC DATE RECOVERY (20240309) ---
        if is_catastrophic:
