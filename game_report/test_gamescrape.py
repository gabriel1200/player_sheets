...
def pull_game_level(dateframe, start_year,end_year,ps=False):
    ...
    for year in range(start_year, end_year):
        ...
        # New variable for max daily retries
        max_daily_retries = 3

        for date_num in year_dates:
            
            # Inner loop for retrying a specific date's pull due to smell check failure
            for daily_attempt in range(max_daily_retries):
                
                try:
                    date_str = format_date_to_url(date_num)
                    
                    print(f"--- Processing Date: {date_num} (Daily Attempt: {daily_attempt + 1}/{max_daily_retries}) ---")

                    # --- 1. Base Stats ---
                    url = f'https://stats.nba.com/stats/leaguedashplayerstats?College=&Conference=&Country=&DateFrom={date_str}&DateTo={date_str}&Division=&DraftPick=&DraftYear=&GameScope=&GameSegment=&Height=&ISTRound=&LastNGames=0&LeagueID=00&Location=&MeasureType=Base&Month=0&OpponentTeamID=0&Outcome=&PORound=&PaceAdjust=N&PerMode=Totals&Period=0&PlayerExperience=&PlayerPosition=&PlusMinus=N&Rank=N&Season={season}&SeasonSegment=&SeasonType={stype}&ShotClockRange=&StarterBench=&TeamID=0&VsConference=&VsDivision=&Weight='
                    df = pull_data(url)

                    # --- 2. Advanced Stats ---
                    url2 = f'https://stats.nba.com/stats/leaguedashplayerstats?College=&Conference=&Country=&DateFrom={date_str}&DateTo={date_str}&Division=&DraftPick=&DraftYear=&GameScope=&GameSegment=&Height=&ISTRound=&LastNGames=0&LeagueID=00&Location=&MeasureType=Advanced&Month=0&OpponentTeamID=0&Outcome=&PORound=&PaceAdjust=N&PerMode=Totals&Period=0&PlayerExperience=&PlayerPosition=&PlusMinus=N&Rank=N&Season={season}&SeasonSegment=&SeasonType={stype}&ShotClockRange=&StarterBench=&TeamID=0&VsConference=&VsDivision=&Weight='
                    df2 = pull_data(url2)

                    # --- 3. Passing Stats (Contains POTENTIAL_AST and AST) ---
                    ...
                    # --- 7. Very Tight Defense Shot Stats ---
                    url7 = f'https://stats.nba.com/stats/leaguedashplayerptshot?CloseDefDistRange=0-2%20Feet%20-%20Very%20Tight&College=&Conference=&Country=&DateFrom={date_str}&DateTo={date_str}&Division=&DraftPick=&DraftYear=&DribbleRange=&GameScope=&GameSegment=&GeneralRange=&Height=&LastNGames=0&LeagueID=00&Location=&Month=0&OpponentTeamID=0&Outcome=&PORound=&PerMode=Totals&Period=0&PlayerExperience=&PlayerPosition=&Season={season}&SeasonSegment=&SeasonType={stype}&ShotClockRange=&ShotDistRange=&StarterBench=&TeamID=0&TouchTimeRange=&VsConference=&VsDivision=&Weight='
                    df7 = pull_data(url7)
                    term = 'very_tight_'
                    if df7 is not None: df7.rename(columns={col: term + col for col in shotcolumns}, inplace=True)

                    # --- 8. Tight Defense Shot Stats ---
                    url8 = 'https://stats.nba.com/stats/leaguedashplayerptshot?CloseDefDistRange=2-4%20Feet%20-%20Tight&College=&Conference=&Country=&DateFrom=' + date_str + '&DateTo=' + date_str + '&Division=&DraftPick=&DraftYear=&DribbleRange=&GameScope=&GameSegment=&GeneralRange=&Height=&LastNGames=0&LeagueID=00&Location=&Month=0&OpponentTeamID=0&Outcome=&PORound=&PerMode=Totals&Period=0&PlayerExperience=&PlayerPosition=&Season=' + season + '&SeasonSegment=&SeasonType='+stype+'&ShotClockRange=&ShotDistRange=&StarterBench=&TeamID=0&TouchTimeRange=&VsConference=&VsDivision=&Weight='
                    df8 = pull_data(url8)
                    term = 'tight_'
                    if df8 is not None: df8.rename(columns={col: term + col for col in shotcolumns},inplace=True)

                    # --- 9. Open Defense Shot Stats ---
                    ...
                    df9 = pull_data(url9)
                    term = 'open_'
                    if df9 is not None: df9.rename(columns={col: term + col for col in shotcolumns},inplace=True)

                    # --- 10. Wide Open Defense Shot Stats ---
                    url10 = 'https://stats.nba.com/stats/leaguedashplayerptshot?CloseDefDistRange=6%2B%20Feet%20-%20Wide%20Open&College=&Conference=&Country=&DateFrom=' + date_str + '&DateTo=' + date_str + '&Division=&DraftPick=&DraftYear=&DribbleRange=&GameScope=&GameSegment=&GeneralRange=&Height=&LastNGames=0&LeagueID=00&Location=&Month=0&OpponentTeamID=0&Outcome=&PORound=&PerMode=Totals&Period=0&PlayerExperience=&PlayerPosition=&Season=' + season + '&SeasonSegment=&SeasonType='+stype+'&ShotClockRange=&ShotDistRange=&StarterBench=&TeamID=0&TouchTimeRange=&VsConference=&VsDivision=&Weight='
                    df10 = pull_data(url10)
                    term = 'wide_open_'
                    if df10 is not None: df10.rename(columns={col: term + col for col in shotcolumns},inplace=True)

                    # --- 11. Pull Up Shot Stats ---
                    ...
                    term='pullup_'
                    if df11 is not None: df11.rename(columns={col: term + col for col in shotcolumns2},inplace=True)
                    ...
                    # --- 13. Shot Locations (By Zone) ---
                    url13=f"https://stats.nba.com/stats/leaguedashplayershotlocations?College=&Conference=&Country=&DateFrom={date_str}&DateTo={date_str}&DistanceRange=By%20Zone&Division=&DraftPick=&DraftYear=&GameScope=&GameSegment=&Height=&ISTRound=&LastNGames=0&Location=&MeasureType=Base&Month=0&OpponentTeamID=0&Outcome=&PORound=0&PaceAdjust=N&PerMode=Totals&Period=0&PlayerExperience=&PlayerPosition=&PlusMinus=N&Rank=N&Season={season}&SeasonSegment=&SeasonType={stype}&ShotClockRange=&StarterBench=&TeamID=0&VsConference=&VsDivision=&Weight="
                    ...
                    if df13 is not None: df13.columns=zone_columns
                    
                    # --- 14. Less Than 6ft Defense Stats ---
                    url14=f"https://stats.nba.com/stats/leaguedashptdefend?College=&Conference=&Country=&DateFrom{date_str}=&DateTo={date_str}&DefenseCategory=Less%20Than%206Ft&Division=&DraftPick=&DraftYear=&GameSegment=&Height=&ISTRound=&LastNGames=0&LeagueID=00&Location=&Month=0&OpponentTeamID=0&Outcome=&PORound=0&PerMode=Totals&Period=0&PlayerExperience=&PlayerPosition=&Season={season}&SeasonSegment=&SeasonType={stype}&StarterBench=&TeamID=0&VsConference=&VsDivision=&Weight="
                    df14=pull_data(url14)
                    if df14 is not None: df14.rename(columns={'CLOSE_DEF_PERSON_ID':'PLAYER_ID'},inplace=True)

                    # --- 15. Shot Locations (By 5ft Range) ---
                    url15=f"https://stats.nba.com/stats/leaguedashplayershotlocations?College=&Conference=&Country=&DateFrom={date_str}&DateTo={date_str}&DistanceRange=5ft%20Range&Division=&DraftPick=&DraftYear=&GameScope=&GameSegment=&Height=&ISTRound=&LastNGames=0&Location=&MeasureType=Base&Month=0&OpponentTeamID=0&Outcome=&PORound=0&PaceAdjust=N&PerMode=Totals&Period=0&PlayerExperience=&PlayerPosition=&PlusMinus=N&Rank=N&Season={season}&SeasonSegment=&SeasonType={stype}&ShotClockRange=&StarterBench=&TeamID=0&VsConference=&VsDivision=&Weight="
                    df15=pull_data(url15)
                    if df15 is not None: df15.columns=['PLAYER_ID', 'PLAYER_NAME', 'TEAM_ID', 'TEAM_ABBR', 'AGE', 'NICKNAME',
                    ...
                    # --- 17. Team Possessions ---
                    url17 = f'https://stats.nba.com/stats/leaguedashteamstats?College=&Conference=&Country=&DateFrom={date_str}&DateTo={date_str}&Division=&DraftPick=&DraftYear=&GameScope=&GameSegment=&Height=&ISTRound=&LastNGames=0&LeagueID=00&Location=&MeasureType=Advanced&Month=0&OpponentTeamID=0&Outcome=&PORound=&PaceAdjust=N&PerMode=Totals&Period=0&PlayerExperience=&PlayerPosition=&PlusMinus=N&Rank=N&Season={season}&SeasonSegment=&SeasonType=Playoffs&ShotClockRange=&StarterBench=&TeamID=0&VsConference=&VsDivision=&Weight='
                    ...
                    if df17 is not None:
                        ...
                        if df is not None: df['team_poss']=df['TEAM_ID'].map(poss_map)

                    # --- 18. Overall Defense Stats ---
                    url18 = f'https://stats.nba.com/stats/leaguedashptdefend?College=&Conference=&Country=&DateFrom={date_str}&DateTo={date_str}&DefenseCategory=Overall&Division=&DraftPick=&DraftYear=&GameSegment=&Height=&ISTRound=&LastNGames=0&LeagueID=00&Location=&Month=0&OpponentTeamID=0&Outcome=&PORound=0&PerMode=PerGame&Period=0&PlayerExperience=&PlayerPosition=&Season={season}&SeasonSegment=&SeasonType={stype}&StarterBench=&TeamID=0&VsConference=&VsDivision=&Weight='
                    df18 = pull_data(url18)
                    if df18 is not None:
                        df18.rename(columns={'CLOSE_DEF_PERSON_ID': 'PLAYER_ID'}, inplace=True)
                        if df8 is not None: df18.rename(columns={col: f'overall_def_{col}' for col in df8.columns if col != 'PLAYER_ID'}, inplace=True)

                    # --- 19. 3-Pointers Defense Stats ---
                    url19 = f'https://stats.nba.com/stats/leaguedashptdefend?College=&Conference=&Country=&DateFrom={date_str}&DateTo={date_str}&DefenseCategory=3%20Pointers&Division=&DraftPick=&DraftYear=&GameSegment=&Height=&ISTRound=&LastNGames=0&LeagueID=00&Location=&Month=0&OpponentTeamID=0&Outcome=&PORound=0&PerMode=PerGame&Period=0&PlayerExperience=&PlayerPosition=&Season={season}&SeasonSegment=&SeasonType={stype}&StarterBench=&TeamID=0&VsConference=&VsDivision=&Weight='
                    ...
                    # --- 20. 2-Pointers Defense Stats ---
                    url20 = f'https://stats.nba.com/stats/leaguedashptdefend?College=&Conference=&Country=&DateFrom={date_str}&DateTo={date_str}&DefenseCategory=2%20Pointers&Division=&DraftPick=&DraftYear=&GameSegment=&Height=&ISTRound=&LastNGames=0&LeagueID=00&Location=&Month=0&OpponentTeamID=0&Outcome=&PORound=0&PerMode=PerGame&Period=0&PlayerExperience=&PlayerPosition=&Season={season}&SeasonSegment=&SeasonType={stype}&StarterBench=&TeamID=0&VsConference=&VsDivision=&Weight='
                    ...
                    # --- 21. Less than 6ft Defense Stats ---
                    url21 = f'https://stats.nba.com/stats/leaguedashptdefend?College=&Conference=&Country=&DateFrom={date_str}&DateTo={date_str}&DefenseCategory=Less%20Than%206Ft&Division=&DraftPick=&DraftYear=&GameSegment=&Height=&ISTRound=&LastNGames=0&LeagueID=00&Location=&Month=0&OpponentTeamID=0&Outcome=&PORound=0&PerMode=PerGame&Period=0&PlayerExperience=&PlayerPosition=&Season={season}&SeasonSegment=&SeasonType={stype}&StarterBench=&TeamID=0&VsConference=&VsDivision=&Weight='
                    ...
                    # --- 22. Less than 10ft Defense Stats ---
                    url22 = f'https://stats.nba.com/stats/leaguedashptdefend?College=&Conference=&Country=&DateFrom={date_str}&DateTo={date_str}&DefenseCategory=Less%20Than%2010Ft&Division=&DraftPick=&DraftYear=&GameSegment=&Height=&ISTRound=&LastNGames=0&LeagueID=00&Location=&Month=0&OpponentTeamID=0&Outcome=&PORound=0&PerMode=PerGame&Period=0&PlayerExperience=&PlayerPosition=&Season={season}&SeasonSegment=&SeasonType={stype}&StarterBench=&TeamID=0&VsConference=&VsDivision=&Weight='
                    ...
                    # --- 23. Greater than 15ft Defense Stats (Issue: URL is for Rebounding, changing to >15ft defense) ---
                    # Original line: df23 = pull_data(url6) - Replaced with correct URL for >15ft defense
                    url23 = f'https://stats.nba.com/stats/leaguedashptdefend?College=&Conference=&Country=&DateFrom={date_str}&DateTo={date_str}&DefenseCategory=Greater%20Than%2015Ft&Division=&DraftPick=&DraftYear=&GameSegment=&Height=&ISTRound=&LastNGames=0&LeagueID=00&Location=&Month=0&OpponentTeamID=0&Outcome=&PORound=0&PerMode=PerGame&Period=0&PlayerExperience=&PlayerPosition=&Season={season}&SeasonSegment=&SeasonType={stype}&StarterBench=&TeamID=0&VsConference=&VsDivision=&Weight='
                    ...
                    # --- 24. Hustle Stats ---
                    url24 = f'https://stats.nba.com/stats/leaguehustlestatsplayer?College=&Conference=&Country=&DateFrom={date_str}&DateTo={date_str}&Division=&DraftPick=&DraftYear=&GameScope=&Height=&ISTRound=&LastNGames=0&LeagueID=00&Location=&Month=0&OpponentTeamID=0&Outcome=&PORound=0&PaceAdjust=N&PerMode=PerGame&PlayerExperience=&PlayerPosition=&PlusMinus=N&Rank=N&Season={season}&SeasonSegment=&SeasonType={stype}&TeamID=0&VsConference=&VsDivision=&Weight='
                    df24 = pull_data(url24)
                    if df24 is not None: df24.rename(columns={col: f'hustle_{col}' for col in df24.columns if col != 'PLAYER_ID'}, inplace=True)
                    ...
                    if df25 is not None: df25.rename(columns={col: f'post_touch_{col}' for col in df25.columns if col != 'PLAYER_ID'}, inplace=True)


                    # --- 26. Speed Distance Stats ---
                    ...
                    # Ensure the critical dataframe (Base df) was pulled
                    if df is None:
                        print(f"Skipping date {date_num} due to failed pull on Base URL.")
                        break # Break the daily_attempt loop, move to the next date
                    ...
                    for frame in frames:
                        if frame is not None:
                            # Drop columns from frame that are already in df, except 'PLAYER_ID'
                            joined_columns = set(frame.columns) - set(df.columns)
                            if 'PLAYER_ID' not in joined_columns and 'PLAYER_ID' in frame.columns:
                                joined_columns.add('PLAYER_ID')
                            
                            joined_columns = [col for col in joined_columns if col in frame.columns]
                            
                            frame_to_merge = frame[joined_columns]
                            df = df.merge(frame_to_merge, on='PLAYER_ID', how='left').reset_index(drop=True)
                    
                    # --- SMELL CHECK LOGIC (Check for NaN POTENTIAL_AST when AST >= 2) ---
                    # Condition: AST >= 2 AND POTENTIAL_AST is NaN
                    
                    # Ensure columns exist before running the check
                    if 'AST' not in df.columns or 'POTENTIAL_AST' not in df.columns:
                         # This should only happen if essential pull (df or df3) fails silently without exception
                         print(f"Skipping smell check for {date_num}: Missing AST or POTENTIAL_AST columns in merged data.")
                         if daily_attempt < max_daily_retries - 1:
                            time.sleep(5)
                            continue 
                         else:
                            break
                    ...
                    smell_players = df[
                        (df['AST'] >= 2) & 
                        (df['POTENTIAL_AST'].isna())
                    ]
                    
                    if not smell_players.empty:
                        player_list = smell_players['PLAYER_NAME'].tolist()
                        print(f"!!! SMELL DETECTED for Date {date_num} !!!")
                        print(f"Players with AST >= 2 and POTENTIAL_AST is NaN: {player_list}")
                        if daily_attempt < max_daily_retries - 1:
                            print(f"Rerunning scrape for date {date_num}...")
                            time.sleep(5) # Wait a bit longer before immediate re-scrape
                            continue # Continue the daily_attempt loop (re-pulls data)
                        else:
                            print(f"Max retries reached for date {date_num}. Accepting potentially bad data.")
                    
                    # If the smell check passes OR max retries are reached:
                    df['year'] = year
                    df['date']=date_num
                    extra_columns = [
                    '_PLAYER_NAME', 
                    ...
                    if count %10==0:
                        ...
                        yeardata.to_csv(f'year_files/{year}{trail}_games.csv',index=False)
                        
                    break # Break the daily_attempt loop, successfully finished this date

                except Exception as e:
                    print(f"General error processing date {date_num} on attempt {daily_attempt + 1}: {e}")
                    time.sleep(1)
                    if daily_attempt == max_daily_retries - 1:
                        break # Break the daily_attempt loop, move to next date
                    else:
                        continue # Continue the daily_attempt loop (re-pulls data)
...
for year in range(start_year,end_year):
    ...
    if os.path.exists(f'year_files/{year}{trail}_games.csv'):
        df = pd.read_csv(f'year_files/{year}{trail}_games.csv')
    else:
        print(f"Skipping final processing for {year}: intermediate file not found.")
    ...
    for date in df['date'].unique().tolist():
        ...
        save_frame.drop_duplicates(inplace=True)
        # Ensure all IDs are integer before saving
    ...
    if games_collected:
        ...
        # Ensure all_games directory exists
