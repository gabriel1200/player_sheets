...
# ---------------------------------------------------------------------
# FAN-OUT DIAGNOSTIC (added to pinpoint the modern.csv duplicate-row bug
# -- see conversation history: 4 player-seasons ended up with 64 rows
# each in modern.csv, salary stayed constant across them but ~33
# defensive-tracking columns didn't, meaning a merge somewhere is fanning
# a single player-season out against a table that has more than one row
# for that player-year. This block traces exactly where.)
#
# Set to any of the 4 known-affected combos to trace this run:
#   (201147, 2018), (202347, 2015), (202706, 2014), (1628969, 2023)
# Leave TARGET_YEAR as None to check across all years (noisier output,
# but useful if you want to confirm the OTHER 3 combos hit the same
# merge point as this one).
# ---------------------------------------------------------------------
TARGET_ID = 201147
TARGET_YEAR = 2018

def check_fanout(df, label):
    id_col = 'PLAYER_ID' if 'PLAYER_ID' in df.columns else ('EntityId' if 'EntityId' in df.columns else None)
    if id_col is None or 'year' not in df.columns:
        print(f"[FANOUT CHECK] [{label}] -- no id/year columns yet, skipping")
        return
    mask = df[id_col].astype(str) == str(TARGET_ID)
    if TARGET_YEAR is not None:
        mask &= (df['year'] == TARGET_YEAR)
    n = mask.sum()
    if n > 1:
        print(f"[FANOUT CHECK] *** [{label}] rows for player {TARGET_ID}"
              f"{f', year {TARGET_YEAR}' if TARGET_YEAR is not None else ''}: {n}  <-- FAN-OUT HERE")
    else:
        print(f"[FANOUT CHECK] [{label}] rows for player {TARGET_ID}"
              f"{f', year {TARGET_YEAR}' if TARGET_YEAR is not None else ''}: {n}")
...
def collect_yeardata(ps=False):
    ...
    for year in range(1997,end_year+1):
        ...
        if year >2000:
            ...
            # --- FANOUT CHECK: is the raw _pbp file itself already
            # carrying more than one row for this player-year, before
            # we even merge it in? ---
            if year == TARGET_YEAR or TARGET_YEAR is None:
                check_fanout(df2, f"raw {year}{trail}_pbp.csv (pre-merge)")
            ...
            if year == TARGET_YEAR or TARGET_YEAR is None:
                check_fanout(df, f"after df2/_pbp merge (year={year})")
...
check_fanout(data, "after collect_yeardata()")
...
check_fanout(data, "after column selection/reorder (pre-save)")
...
# --- FANOUT CHECK: does the salaries source itself have more than one
# row for this player-year? ---
check_fanout(salaries, "raw year_salaries.csv (pre-merge)")
...
check_fanout(totals, "after totals = pd.read_csv(total_path)")
...
for year in range(start_year,end_year+1):
    ...
    if year == TARGET_YEAR or TARGET_YEAR is None:
        check_fanout(yearframe, f"yearframe after totals[year=={year}] filter")
    ...
    if trail =='' and year>=2010:
        ...
        if year == TARGET_YEAR or TARGET_YEAR is None:
            # is lebronyear itself carrying duplicate rows for this player before the merge?
            dupe_rows = lebronyear[lebronyear['PLAYER_ID'].astype(str) == str(TARGET_ID)]
            if len(dupe_rows) > 1:
                print(f"[FANOUT CHECK] *** lebronyear (year={year}) has {len(dupe_rows)} "
                      f"rows for player {TARGET_ID} BEFORE merge  <-- FAN-OUT HERE")

        yearframe=yearframe.merge(lebronyear,on=['PLAYER_ID','year'],how='left')

        if year == TARGET_YEAR or TARGET_YEAR is None:
            check_fanout(yearframe, f"after lebron merge (year={year})")
        ...
        if len(lebronyear)==0:
            ...
            if len(temp_index)>0:
                ...
                if year == TARGET_YEAR or TARGET_YEAR is None:
                    dupe_rows = temp_index[temp_index['PLAYER_ID'].astype(str) == str(TARGET_ID)]
                    if len(dupe_rows) > 1:
                        print(f"[FANOUT CHECK] *** temp_index (year={year}) has {len(dupe_rows)} "
                              f"rows for player {TARGET_ID} BEFORE merge (note: this merge has "
                              f"NO 'year' key, only PLAYER_ID -- if temp_index has this player "
                              f"more than once across ANY year, EVERY row fans out here)  <-- CHECK THIS")

                yearframe = yearframe.merge(temp_index, on='PLAYER_ID')

                if year == TARGET_YEAR or TARGET_YEAR is None:
                    check_fanout(yearframe, f"after temp_index merge (year={year})")
    ...
    if year >= 2014 and trail == '':
        ...
        if year == TARGET_YEAR or TARGET_YEAR is None:
            dupe_rows = salary_year[salary_year['PLAYER_ID'].astype(str) == str(TARGET_ID)]
            if len(dupe_rows) > 1:
                print(f"[FANOUT CHECK] *** salary_year (year={year}) has {len(dupe_rows)} "
                      f"rows for player {TARGET_ID} BEFORE merge  <-- FAN-OUT HERE")
        ...
        if year == TARGET_YEAR or TARGET_YEAR is None:
            check_fanout(yearframe, f"after salary merge (year={year})")
        ...
        if year == TARGET_YEAR or TARGET_YEAR is None:
            check_fanout(yearframe, f"after cap merge (year={year})")
        ...
        if trail == '_ps':
            ...
            try:
                ...
                if 'Pos' in reg_season_df.columns:
                    ...
                    if year == TARGET_YEAR or TARGET_YEAR is None:
                        check_fanout(yearframe, f"after player_positions merge (year={year}, ps branch)")
    ...
    if year == TARGET_YEAR or TARGET_YEAR is None:
        check_fanout(yearframe, f"final yearframe for year={year} (pre-save)")
...
check_fanout(modern, "after final modern = pd.concat(modern_years)")
